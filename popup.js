...
chrome.storage.sync.get("searchkeyword", function(result) {
  if (result.searchkeyword === undefined) {
    ...
    chrome.storage.sync.set({ "searchkeyword": 'Rocking' }, function() {
    });
...
chrome.storage.sync.get("locale", function(result) {
  if (result.locale === undefined) {
    ...
    chrome.storage.sync.set({ "locale": 'th' }, function() {
    });
  }else{
...
chrome.storage.sync.get("interval", function(result) {
  if (result.interval === undefined) {
    ...
    chrome.storage.sync.set({ "interval": 5 }, function() {
    });
  }else {
...
chrome.storage.sync.get("automate", function(result) {
  if (result.automate === undefined) {
    ...
    chrome.storage.sync.set({ "automate": true }, function() {
    });
  }else {
...
searchkeywordInput.onchange = (e) => {
  ...
  chrome.storage.sync.set({ "searchkeyword": value }, function() {
    console.log('searchkeyword is set to ' + value);
  });
}
localeInput.onchange = (e) => {
  const value = e.target.value
  chrome.storage.sync.set({ "locale": value }, function() {
...
hmaAutomate.onchange = (e) => {
  ...
  chrome.storage.sync.set({ "automate": value }, function() {
...
msaHandleEle.onclick = (e) => {
  if (e.target.dataset.status === 'true' && e.target.dataset.status){
    chrome.storage.sync.set({ "scripthandle": false }, function() {
      console.log('interval is set to False');
    });

    chrome.storage.sync.set({ "message": 'Script starting' }, function() {
    });

    chrome.storage.sync.set({ "uriResults": null }, function() {
    });
    ...
    chrome.storage.sync.set({ "message": 'Script stopping' }, function() {
    });
...
const product_automate = (url) => {
  chrome.tabs.create({ url: `https://hermes.com${url}` }, tab => {
